  ...
  const fetchLocation = async () => {
    try {
      ...
      if (data.city && data.country_name) {
        setOutput((prev) => [...prev, "location --current", `Visitor from ${data.city}, ${data.country_name}`])
        setLocation(`${data.city}, ${data.country_name}`)
      }
    } catch (error) {
      setOutput((prev) => [...prev, "location --current", "Location data unavailable"])
    }
  }
    ...
    if (e.key === "Enter") {
      ...
      if (command) {
        ...
        if (command === "clear") {
          ...
          response = "Available commands: whoami, skills, projects, experience, location, clear, help"
        } else if (command === "whoami") {
          response = "Semeton Balogun - Backend Engineer & Technical Lead"
        } else if (command === "skills") {
          response = "PHP (Laravel) | Node.js | System Architecture | Security | Docker | CI/CD"
        } else if (command === "projects") {
          response = "Trustcenta | Tangerine LMS | Trustpass | ScholarshipIQ | Verivault"
        } else if (command === "experience") {
          response = "5 years of professional experience in backend development and technical leadership"
        } else if (command === "location") {
          response = location ? `Visitor from ${location}` : "Location data unavailable"
        }
    ...
    <Card className="border border-gray-200 dark:border-gray-800 bg-gray-900 text-gray-100 p-4 w-full max-w-3xl mx-auto rounded-lg shadow-lg overflow-hidden">
      ...
      >
        {output.map((line, index) => (
          <div key={index} className={isCommand(line) ? "text-green-400 mt-2" : "text-gray-300 mb-1"}>
            {line}
          </div>
        ))}
        <div className="flex items-center mt-2">
          ...
          <span className={`ml-0.5 w-2 h-4 bg-gray-300 ${cursorVisible ? "opacity-100" : "opacity-0"}`}></span>
        </div>
      </div>
      ...
      <div className="mt-2 text-xs text-gray-500">
        Type <span className="text-gray-400 font-semibold">help</span> for available commands
      </div>
    </Card>
  )
}
